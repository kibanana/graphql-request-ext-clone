...
function activate(context) {
    context.subscriptions.push(vscode.commands.registerCommand('extension.helloGraphQL', () => __awaiter(this, void 0, void 0, function* () {
        queryInput_1.showInputBoxMine.call(() => {
        }).then(data => {
            console.log(`${data[0]}, ${data[1]}, ${data[2]}`);
            makeWebview(data[0], data[1], data[2]);
        });
    })));
}
...
function makeWebview(URL, QUERY, VARIABLE) {
    if (!(URL && QUERY)) {
        vscode.window.showErrorMessage(`Can't Execute Query!`);
        return;
    }
